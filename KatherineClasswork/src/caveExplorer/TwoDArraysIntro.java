...
public class TwoDArraysIntro {
	...
	static int starti;
	static int startj;
	static int treasurei = 4;
	static int treasurej = 3;
	...
	public static void makeMap(){
		...
		for(int row = 0; row < map.length; row++){
			for(int col = 0; col < map[row].length; col++){
				if(row%3 == 0 || row == map.length - 1){
					map[row][col] = "_";
				}
				if(row != 0 && col%6 == 0 || row != 0 && col == map[row].length){
					map[row][col] = "|";
				}
			}
		}
	...
	private static int[] interpretInput(String input) {
		...
		if(currenti >= 0 && currenti < arr2D.length && currentj >= 0 && currentj < arr2D[0].length){
			...
			System.out.println("Sorry, you've reach the edge of the unknown universe. You may go no farther in that direction");
		}
...
//			}
	...
	public static void practiceImage(){
